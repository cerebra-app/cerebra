...
/* ---------------------------------------------------------- */
function DeckDetail({ session, deck, cards, autoGenerateDocId, onCardsChanged, onRefresh, onBack, onStartStudy, dueCount, toast }) {
  ...
  return (
    <div className="flex flex-col items-center py-6">
      ...
      <p className="text-sm text-slate-400">
        {total - correct} to review again next time
      </p>
    </div>
  );
}
...
function StudySession({ dueCards, onFinish, toast }) {
  ...
  const [results, setResults] = useState({}); // { [cardId]: boolean }
  const [finished, setFinished] = useState(false);

  const card = dueCards[index];
  const isLast = index === dueCards.length - 1;

  const handleRate = async (correct) => {
    setRating(true);
  ...
  if (finished) {
    ...
    return (
      <div className="animate-fade-in">
        <ResultsRing correct={correctCount} total={dueCards.length} />
  ...
  const alreadyAnswered = results[card.id] !== undefined;
    ...
    <div className="animate-fade-in">
      ...
      {!flipped ? (
        <div className="flex gap-2">
          ...
          <Button size="lg" className="flex-1" onClick={() => setFlipped(true)}>
            Show answer
          </Button>
        </div>
      ) : (
        <div className="flex gap-2">
          ...
          >
            ✗ {alreadyAnswered ? "Still wrong" : "Wrong"}
          </Button>
          <Button
          ...
          >
            ✓ {alreadyAnswered ? "Still correct" : "Correct"}
          </Button>
        </div>
      )}
    </div>
      ...
      {screen === "detail" && activeDeck && (
        <DeckDetail
          ...
          onCardsChanged={(next) => {
            setCards(next);
            const nowIso = new Date().toISOString();
            const dueCount = next.filter((c) => c.next_review_at <= nowIso).length;
            setDecks((prev) =>
              prev.map((d) =>
                d.id === activeDeck.id ? { ...d, card_count: next.length } : d
              )
            );
            setDueCounts((prev) => ({ ...prev, [activeDeck.id]: dueCount }));
          }}
          onRefresh={async () => {
            const { data, error } = await supabase
              .from("flashcards")
            ...
            setCards(fresh);
            const nowIso = new Date().toISOString();
            const dueCount = fresh.filter((c) => c.next_review_at <= nowIso).length;
            setDecks((prev) =>
              prev.map((d) =>
                d.id === activeDeck.id ? { ...d, card_count: fresh.length } : d
              )
            );
            setDueCounts((prev) => ({ ...prev, [activeDeck.id]: dueCount }));
          }}
